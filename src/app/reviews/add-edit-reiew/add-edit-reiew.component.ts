...
@Component({
  selector: 'app-add-edit-reiew',
  templateUrl: './add-edit-reiew.component.html',
  styleUrls: ['./add-edit-reiew.component.scss']
})
export class AddEditReiewComponent implements OnInit, OnDestroy {
  form: FormGroup
  formValues: any
  submitting = false
  hasError = false
  ...
  submitForm() {
    ...
    this.subs.add(
      this.reviewService.createReviews(params).subscribe(data => {
        if (data) {
          this.submitting = false
          Swal.fire(
            {
        ...
        if (error) {
          ...
          this.submitting = false
          this.hasError = true
          this.errorMsg = 'Something went wrong while trying to create your review!! '
        }
      })
    )
  }
